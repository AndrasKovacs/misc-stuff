...
data Sum :: forall (ks :: [*]). IxNP ks -> Ixed (HList ks) where
  Here  :: (f :: Ixed k) i j a -> Sum' (k ': ks) (f ::: fs) (i :> is) (j :> is) a
  There :: Sum' ks fs is js a -> Sum' (k ': ks) (f ::: fs) (i :> is) (i :> js) a
...
instance (ks ~ (k ': ks'), fs ~~ (f ::: fs'), is ~~ (i :> is'), js ~~ (j :> is')) =>
  Elem' Z k ks f fs i is j js where
  inj'          = Here
  prj' (Here f) = Just f
  prj' _        = Nothing

instance
  (Elem' n k ks' f fs' i is' j js',
   ks ~ (k' ': ks'), fs ~~ (f' ::: fs'), is ~~ (i' :> is'), js ~~ (i' :> js')) =>
  Elem' (S n) k ks f fs i is j js where
  inj'            = There . inj' @n
  prj' (Here _)   = Nothing
...
--  Impure (There (Here Open)) (Leaf Pure)
