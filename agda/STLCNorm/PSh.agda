...
lem :
  ∀ {Γ Δ A}(t : Tm Γ A)(σ : Tms Γ Δ)
  → Tms↑ wk idₛ ∘ (σ , t) ≡ idₛ ∘ σ
lem {Δ = ∙} t σ = refl
lem {Δ = Δ , x} t (σ , h) = {!!}
...
data _~>_ {Γ} : ∀ {A} → Tm Γ A → Tm Γ A → Set where
  ...
  lam  : ∀ {A B}{t : Tm (Γ , A) B} t'      → t ~> t' →  lam t          ~> lam t'
...
_~>*++_ : ∀ {Γ A}{t t' t'' : Tm Γ A} → t ~>* t' → t' ~>* t'' → t ~>* t''
∙          ~>*++ q = q
(step ∷ p) ~>*++ q = step ∷ (p ~>*++ q)
infixr 5 _~>*++_
...
⟦Ty⟧~> {ι}     step (n , p) = n , ((step ∷ ∙ ) ~>*++ p)
⟦Ty⟧~> {A ⇒ B} step tᴹ      = λ Σ r a aᴹ → ⟦Ty⟧~> (app₁ _ (Tm↑~> r step)) (tᴹ Σ r a aᴹ)
...
⟦ lam t   ⟧Tm σ σᴹ = λ Σ r a aᴹ
  → ⟦Ty⟧~>
    (coe (ap ((app (lam (Tm↑ (keep r) (t [ σ ^ _ ]))) a ~>_))
      (
...
⟦_⟧Tms : ∀ {Γ Δ}(σ : Tms Γ Δ) → ∀ {Σ}(δ : Tms Σ Γ) → ⟦ Γ ⟧Con δ → ⟦ Δ ⟧Con (σ ∘ δ)
⟦ ∙     ⟧Tms δ _  = tt
⟦ σ , t ⟧Tms δ δᴹ = ⟦ σ ⟧Tms δ δᴹ , ⟦ t ⟧Tm δ δᴹ

lam~>* : ∀ {Γ A B}{t t' : Tm (Γ , A) B} → t ~>* t' → lam t ~>* lam t'
lam~>* ∙          = ∙
lam~>* (step ∷ p) = lam _ step ∷ lam~>* p

mutual
   q : ∀ A {Γ}{t : Tm Γ A} → ⟦ A ⟧Ty t → Σ (Nf Γ A) λ n → t ~>* ⌜ n ⌝ 
   q ι       {Γ} {t} tᴹ = tᴹ
   q (A ⇒ B) {Γ} {t} tᴹ =
     lamₙ (proj₁ next) , {!proj₂ next!}
     where next = q B (tᴹ (Γ , A) (add idᵣ) (var vz) (u A (var vz)))

   u : ∀ A {Γ} → (n : Ne Γ A) → ⟦ A ⟧Ty ⌜ ne n ⌝
   u ι       n = ne n , ∙
   u (A ⇒ B) n = λ Δ r a aᴹ → {!!}
