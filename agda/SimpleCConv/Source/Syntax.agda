...
data _~_ {Γ} : ∀ {A} → Tm Γ A → Tm Γ A → Set where
  β     : ∀ {A B}(t : Tm (Γ ▶ A) B) (t' : Tm Γ A) → app (lam t) t' ~ Tmₛ (idₛ , t') t
  η     : ∀ {A B}{t t' : Tm Γ (A ⇒ B)} → app (Tmₑ wk t) (var vz) ~ app (Tmₑ wk t') (var vz) → t ~ t'
  lam   : ∀ {A B}{t t' : Tm (Γ ▶ A) B}  → t ~ t' →  lam {Γ} t ~ lam t'
...
~ₑ σ (β t t') rewrite Tm-ₛ∘ₑ (idₛ , t') σ t ⁻¹ =
...
Tmₛ~t p (lam t) = lam (Tmₛ~t (keepₛ~ p) t)
...
Tmₛσ~ σ (β t t') =
  ...
  ~◾ ≡~ (Tm-∘ₛ _ _ t ⁻¹)
  ~◾ ≡~ ((λ x → Tmₛ (x , Tmₛ σ t') t) &
      (assₛₑₛ σ wk (idₛ , Tmₛ σ t') ◾ ((σ ∘ₛ_) & (idlₑₛ idₛ) ◾ idrₛ σ) ◾ idlₛ σ ⁻¹))
         ~◾ ≡~ (Tm-∘ₛ _ _ t)
...
... | hyp rewrite Tm-ₑ∘ₛ (wk{A}) (keepₛ σ) t ⁻¹ | Tm-ₑ∘ₛ (wk{A}) (keepₛ σ) t' ⁻¹
          ...
          = η hyp
Tmₛσ~ σ (lam p) = lam (Tmₛσ~ (keepₛ σ) p)
