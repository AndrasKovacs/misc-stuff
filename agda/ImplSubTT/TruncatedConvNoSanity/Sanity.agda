...
mutual
  ...
  Γ⊢t⇓Aₑ {σ = σ} σw (A' , tw , p) =
    Tyₑ σ A' , Γ⊢t⇑Aₑ σw tw , ∣∣-rec (embed ∘ ~ₜₑ σ) trunc p
...
mutual
  ...
  Γ⊢t⇑Aₛ σw (lam Aw tw) = lam (Γ⊢Aₛ σw Aw) (Γ⊢t⇑Aₛ (keepₛ⊢ σw) tw)
  Γ⊢t⇑Aₛ {Γ = Γ} {σ = σ} σw (app {t} {u} {A} {B} tw uw) =
    coe ((Γ ⊢ app (Tmₛ σ t) (Tmₛ σ u) ⇑_) &
            ((Ty-∘ₛ (keepₛ σ) (idₛ , Tmₛ σ u) B ⁻¹
          ◾ (λ x → Tyₛ (x , Tmₛ σ u) B) & (assₛₑₛ σ wk (idₛ , Tmₛ σ u)
          ◾ (σ ∘ₛ_) & idlₑₛ idₛ
          ◾ idrₛ σ ◾ idlₛ σ ⁻¹))
          ◾ Ty-∘ₛ (idₛ , u) σ B))
  ...
  Γ⊢t⇓Aₛ : ∀ {n m Γ Δ t A σ} → Sub⊢ {n} Γ {m} Δ σ → Δ ⊢ t ⇓ A → Γ ⊢ Tmₛ σ t ⇓ Tyₛ σ A
  Γ⊢t⇓Aₛ {σ = σ} σw (A' , tw , p) =
    Tyₛ σ A' , Γ⊢t⇑Aₛ σw tw , ∣∣-rec (embed ∘ ~ₜₛ σ) trunc p
