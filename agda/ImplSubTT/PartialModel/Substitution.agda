...
mutual
  ...
  Tmₛ σ (app A B f a) = app (Tyₛ σ A) (Tyₛ (keepₛ σ) B) (Tmₛ σ f) (Tmₛ σ a)
...
mutual
  ...
  Tm-ₛ∘ₑ σ δ (app A B f a) =
    app & Ty-ₛ∘ₑ σ δ A
       ⊗ ((λ x → Tyₛ (x , var zero) B) &
             (assₛₑₑ σ δ wk ◾ (λ x → σ ₛ∘ₑ drop x) & (idrₑ δ ◾ idlₑ δ ⁻¹)
           ◾ assₛₑₑ σ wk (keep δ) ⁻¹ )
...
mutual
  ...
  Tm-idₛ (app A B f a) = app & Ty-idₛ A ⊗ Ty-idₛ B ⊗ Tm-idₛ f ⊗ Tm-idₛ a
