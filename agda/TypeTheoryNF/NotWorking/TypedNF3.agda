...
data Con : ℕ → Set
data _⊢_type {n}(Γ : Con n) : Ty n → Set
data _⊢_∋_ {n}(Γ : Con n) : Tm n → ∀ {A} → Γ ⊢ A type → Set
data _⊢ₙ_∋_ {n}(Γ : Con n) : Ne n → ∀ {A} → Γ ⊢ A type → Set

data Con where
  ε   : Con 0
  _,_ : ∀ {n} (Γ : Con n){a} → Γ ⊢ a type → Con (suc n)

-- wkTy' : ∀ {n}{Γ : Con n} → Ix n → 

-- lookup : ∀ {n} → Con n → Var n → Ty n
-- lookup (_,_ Γ {a} _) (_ , s≤s z≤n)     = wkTy (, z≤n) a
-- lookup (Γ , A)       (_ , s≤s (s≤s v)) = wkTy (, z≤n) (lookup Γ (, s≤s v))

data _⊢_type {n} Γ where
  U      : Γ ⊢ U type
  El     : ∀ {t} → Γ ⊢ t ∋ U → Γ ⊢ El t type
  _⇒_    : ∀ {A B} → Γ ⊢ A type → Γ ⊢ B type → Γ ⊢ (A ⇒ B) type

data _⊢_∋_ {n}(Γ : Con n) where
  ne : ∀ {a}{A : Γ ⊢ a type}{n} → Γ ⊢ₙ n ∋ A → Γ ⊢ ne n ∋ A
  ƛ  : ∀ {a b}{A : Γ ⊢ a type}{B : Γ ⊢ b type}{t}
       → (Γ , A) ⊢ t ∋ {!B!} → Γ ⊢ ƛ a t ∋ (A ⇒ B)

data _⊢ₙ_∋_ {n}(Γ : Con n) where
  var : ∀ {v} → Γ ⊢ₙ var v ∋ {!!}
  _∙_ :
    ∀ {a b}{A : Γ ⊢ a type}{B : Γ ⊢ b type}{f x}
    → Γ ⊢ₙ f ∋ (A ⇒ B) → Γ ⊢ x ∋ A → Γ ⊢ₙ (f ∙ x) ∋ B
